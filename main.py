...
def procedural_generator() -> (arcade.SpriteList, arcade.SpriteList):
    ...
    platform.center_y = rng.randint(800, 2000)
    ...
    while current_y > 85:
        ...
        if rng.random() < 0.2:
            ...
            generated_y = rng.randint(current_y - 90, current_y - 50)
...
class Game(arcade.View):
    ...
    def on_update(self, delta_time: float):
        ...
        if self.a_pressed and not self.d_pressed:
            ...
            if self.player_sprite.change_x > C.FRICTION * self.player_sprite.speed_multiplier + self.player_sprite.momentum:
                self.player_sprite.change_x -= C.FRICTION * self.player_sprite.speed_multiplier
            elif self.player_sprite.change_x < - C.FRICTION * self.player_sprite.speed_multiplier + self.player_sprite.momentum:
                self.player_sprite.change_x += C.FRICTION * self.player_sprite.speed_multiplier
            else:
                if self.player_sprite.momentum != 0:
                    self.player_sprite.change_x = self.player_sprite.momentum
                else:
        ...
        for platform in self.platforms:
            ...
            if platform.boundary_right and platform.center_x > platform.boundary_right and platform.change_x > 0:
                platform.change_x *= -1
            if platform.boundary_left and platform.center_x < platform.boundary_left and platform.change_x < 0:
                platform.change_x *= -1
            if platform.boundary_bottom and platform.center_y < platform.boundary_bottom and platform.change_y < 0:
                platform.change_y *= -1
            if platform.boundary_top and platform.center_y > platform.boundary_top and platform.change_y > 0:
                platform.change_y *= -1
